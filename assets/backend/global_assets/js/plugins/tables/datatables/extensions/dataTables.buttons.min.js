    ...
    i.Api.register("buttons.exportData()", function(a) {
        if (this.context.length) {
            for (var b = new i.Api(this.context[0]), c = d.extend(!0, {}, {
                    ...
                    }).toArray(), /*BEGIN ADD*/headerMatrix = getHeaders( b.settings()[0] )/*END ADD*/, g = b.table().footer() ? b.columns(c.columns).indexes().map(function(a) {
                        var d = b.column(a).footer();
                        return c.format.footer(d ? d.innerHTML : "", a)
                    }).toArray() : null, h =
                    b.rows(c.rows, c.modifier).indexes().toArray(), h = b.cells(h, c.columns).render(c.orthogonal).toArray(), f = a.length, f = headerMatrix[headerMatrix.length - 1].length, k = 0 < f ? h.length / f : 0, j = Array(k), m = 0, l = 0; l < k; l++) {
